...
module.exports = function (options , tmpDir) {
    ...
    this.playlistFile = options.pathname.split('/').pop();
        ...
        stats = fs.stat(tmpDir + '/' + fileUrl, function (err, stat){
          if(err === null) {
            ...
            cb(self);
              ...
              var urlSegment = url.resolve(self.options.href, fileUrl);
              var options = url.parse(urlSegment)
              var writeStream = fs.createWriteStream(options.pathname.split('/').pop() + '.download');
              console.log('New Segment to download ' + urlSegment);
